var Page = {
    ...
    log: function(text) {
        ...
        if (Page.$log.length) {
            Page.$log
                ...
                .scrollTop(Page.$log[0].scrollHeight);
            Page.logIndex++;
        }
    },
    ...
    loadImgToCanvas: function() {
        ...
        // 加载图片到canvas
        var c = CL.loadImgToCanvas(this.obj.img, {
            width: width,
            viewWidth: viewSize,
            backgroundColor: '#fff'
        });
        ...
        $.extend(this.obj, c, o);
        this.obj.canvas.cover = CL.createCanvas({
            width: c.width,
            height: c.height,
            viewWidth: viewSize,
            name: 'cover'
        ...
        if (this.obj.$workbench) {
            ...
            this.obj.$workbench
                .find('.content')
                .empty()
                .append(this.obj.canvas.img, this.obj.canvas.cover);
        }
    },
        ...
        if (file) {
            CL.openImgFile(file, function(err, img) {
                ...
                Page.obj.$workbench = Page.createImgFigure({
                    ...
                    .css({
                        display: 'none',
                        'margin-left': (cWidth - Page.setting.viewSize - 16) / 2
                    })
                    .appendTo('.js_panel')
                    .fadeIn()
                    .delay(500)
                ...
                Page.obj.$workbench.append(
                    $('<div>')
                        .addClass('console js_console')
                        .hide()
                );
            });
        }
    ...
    analyse: function() {
        ...
        CL.analyse(
            ...
            $.extend(
                {
                    onprogress: function() {
                        console.log('>', new Date() - time, new Date() - time2);
                        time2 = new Date();
                    }
                },
                Page.setting
            ...
            function(error, data) {
                ...
                if (data && data.list) {
                    ...
                    $('.js_result h3')
                        .empty()
                        .append('分割出图片数：' + data.list.length);
                    $('.js_result h4').html(
                        '总耗时：' +
                            allTime +
                            'ms / 解析耗时：' +
    ...
    init: function() {
        if (!window.FileReader) {
            ...
            var _togglePanel = function() {
                var $curSet = $(
                    '.js_set .js_item[data-part="' + Page.setting.type + '"]'
                ).show();
                $('.js_set .js_item[data-part]')
                    .not($curSet)
                    .hide();
            };
            ...
            $(document)
                // 选择文件
                .on('dragover dragleave dragend', function(e) {
                    return false;
                })
                .on('drop', function(e) {
                ...
                .on('change', '.js_setting', function(e) {
                    ...
                    if (/^(?:range|number)$/.test(this.type)) {
                        ...
                        value = 0;
                        $('.js_setting[data-name="bgtype"]:checked').each(
                            function() {
                                if (this.value > 0) {
